...
${articles.length > 0 ? '\nI found some relevant articles that might help:' : ''}`
    ...
    if (lowerQuery.includes('cfo')) {
      return `A **Interim CFO** is a part-time Chief Financial Officer who provides strategic financial leadership to multiple companies. They handle:
...
Many interim executives work with 2-3 clients simultaneously.`
  }

  if (lowerQuery.includes('london')) {
    return `**London** is the UK's largest market for interim executives, with strong demand across:
...
Day rates in London are typically 15-20% higher than the rest of the UK. Many London-based interim executives also serve clients remotely across the UK and Europe.
...
${articles.length > 0 ? '\nThese articles provide more detailed guidance:' : ''}`
  ...
  if (lowerQuery.includes('article') || lowerQuery.includes('show') || lowerQuery.includes('read')) {
    ...
    return `I couldn't find specific articles matching your query. Try browsing our [articles page](/interim-jobs-articles) for all our content.`
  }
  ...
  return `I'd be happy to help you learn more about interim executive work! You can ask me about:

- **Roles** - CFO, CMO, CTO, COO, HR Director positions
- **Day rates** - What interim executives typically earn
- **Markets** - London, Manchester, remote opportunities
- **Career transition** - How to become a interim executive
