...
async function searchJobs(params: {
  ...
  try {
    ...
    if (jobs.length === 0) {
      ...
      return `No ${roleText}interim roles found${locationText} currently. Try a broader search.`
    }
    ...
    const jobDescriptions = jobs.map(j => {
      ...
      if (j.slug) desc += ` - View at interim.quest/job/${j.slug}`
      return desc
    }).join('. ')
...
async function searchArticles(params: {
  ...
  try {
    ...
    const articleDescriptions = articles.map(a => {
      ...
      if (a.slug) desc += ` - Read at interim.quest/articles/${a.slug}`
      return desc
    }).join('. ')
