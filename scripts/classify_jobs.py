...
API_BASE_URL = os.environ.get('API_BASE_URL', 'https://interim.quest')
...
class StructuredJob(BaseModel):
    ...
    opportunity_description: str = Field(description="""
        ...
        - General fractional → /fractional-jobs
          Link text variations: "interim jobs", "fractional roles", "interim executive",
...
agent = Agent(
    ...
    system_prompt="""You are the senior content editor for Interim.Quest, the UK's premier platform for interim executive opportunities.
...
- /fractional-jobs (interim jobs, fractional roles, portfolio career, part-time executive)
